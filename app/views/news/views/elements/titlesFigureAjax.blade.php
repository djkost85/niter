<div class="item row">
	@foreach($array as $k => $v)
		<figure class="col-sm-2 home-trailer-poster desktop-only">
			...
			<figcaption><a href="#" title="{{$v->background}}" realtitle="{{ $v->title . ' (' . $v->year . ')' }}" plot="{{ $v->plot }}" details="{{ ucfirst(str_replace("|", ",",$v->genre)) . ' | ' . $v->runtime . ' min.' }}" link="{{ Helpers::url($v->title, $v->id, $v->type) }}">{{ $v->title }}</a></figcaption>                     
		</figure> 
