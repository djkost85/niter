@if ( isset($data) && ! $data->isEmpty() )
	<div class="browse-grid">
		@foreach($data->slice(0,12) as $k => $r)
			@if ($r->type == 'movie')
				<figure class="col-xs-6 col-sm-2" data-filter-class='{{ Helpers::genreFilter($r->genre) }}' data-popularity="{{ ($r['imdb_votes_num'] ? $r['imdb_votes_num'] : $r['tmdb_popularity']) }}" data-name="{{{ $r->title }}}" data-release="{{{ $r->year }}}">
